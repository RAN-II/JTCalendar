...
- (void)updateContentViewWithDate:(NSDate *)contentDate {
	if (!_manager.settings.weekModeEnabled) {
		...
		if (_dayContentView) {
			...
			_dayContentView.frame = CGRectMake(_dayContentView.frame.origin.x, self.frame.size.height, _dayContentView.frame.size.width, _dayContentView.frame.size.height);
		}
		[self layoutSubviews];
...
- (void)layoutSubviews {
	...
	NSInteger numberOfWeeksMax = MIN(_manager.settings.pageViewNumberOfWeeks, MAX_WEEKS_BY_MONTH);
	if (numberOfWeeksMax == 0) {
		numberOfWeeksMax = [_manager.dateHelper numberOfWeeks:_date];
	}
	...
	if (_manager.settings.pageViewHaveWeekDaysView) {
		...
		if (weekDayHeight == 0) { // Or use the same height than weeksViews
			weekDayHeight = self.frame.size.height / (numberOfWeeksMax + 1);
		}
	...
	CGFloat weekHeight = (self.frame.size.height - y) / numberOfWeeksMax;
